          ...
          <div className="p-2 md:p-3">
            ...
            <nav aria-label="Property type filter" className="hidden md:block">
              <Tabs value={activeType} onValueChange={setActiveType}>
                <TabsList className="w-full flex flex-wrap items-center justify-start gap-2 rounded-full bg-muted/60 p-1 border border-border/60 backdrop-blur supports-[backdrop-filter]:bg-muted/40 shadow-sm overflow-visible">
                  {availableTypes.map((type) => (
                    <TabsTrigger
                      key={type}
                      value={type}
