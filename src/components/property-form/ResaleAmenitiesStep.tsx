...
import { Plus, Minus, PawPrint, Dumbbell, UtensilsCrossed, Shield, MoveUp, Wifi, AirVent, MessageCircle, Users, Waves, Flame, Car, Building2, Droplets, Wrench, Trees, Trash2, Tv, Zap, ShieldCheck, ShoppingCart, Accessibility, Bath, PersonStanding, Sparkles } from 'lucide-react';
...
export const ResaleAmenitiesStep: React.FC<ResaleAmenitiesStepProps> = ({
  ...
  const form = useForm<ResaleAmenitiesForm>({
    ...
    defaultValues: {
      ...
      whoWillShow: initialData.whoWillShow ?? 'i-will-show',
      currentPropertyCondition: initialData.currentPropertyCondition ?? 'good',
  ...
  const onSubmit = (data: ResaleAmenitiesForm) => {
    console.log('ResaleAmenitiesStep submitting data:', data);
    onNext(data as PropertyAmenities);
  };
  return <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
      <h1 className="text-2xl mb-6 font-semibold text-red-600">Amenities</h1>
      ...
      <Form {...form}>
        <form id={formId} onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          field
                ...
                <FormLabel>
                  Add Directions Tip for your buyers <span className="bg-red-500 text-white text-xs px-2 py-1 rounded ml-2">NEW</span>
                </FormLabel>
                <FormControl>
