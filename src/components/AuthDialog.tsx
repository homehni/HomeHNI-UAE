...
import { Chrome, LogIn, UserPlus, Eye, EyeOff, X } from 'lucide-react';
...
export const AuthDialog: React.FC<AuthDialogProps> = ({
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      setSignInMessage({ type: 'error', text: error.message || "Failed to sign in with Google. Please try again." });
    }
  };
  ...
  const handleEmailSignIn = async (e: React.FormEvent) => {
    ...
    if (!signInForm.email || !signInForm.password) {
      setSignInMessage({ type: 'error', text: "Please enter both email and password." });
      return;
    }
  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    ...
    if (!signUpForm.fullName || !signUpForm.email || !signUpForm.password) {
      setSignUpMessage({ type: 'error', text: "Please fill in all required fields." });
      return;
    }

    if (signUpForm.password !== signUpForm.confirmPassword) {
      setSignUpMessage({ type: 'error', text: "Passwords do not match. Please try again." });
      return;
    }
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-sm p-0 bg-white/95 backdrop-blur-sm border-2 border-brand-red/30 shadow-2xl shadow-brand-red/10 rounded-2xl">
          {/* Close Button */}
          <Button
            variant="ghost"
            size="sm"
