        ...
        <div className="w-full px-4 lg:px-6 xl:px-8 pt-[6px]">
          <div className="flex justify-between items-center h-14">
            ...
            <div className="flex items-center space-x-4">
              ...
              {/* Location Selector - Hidden on Painting Cleaning, and Packers Movers page */}
              {!isPaintingCleaningPage && !isPackersMoversPage && (
                <Select>
              ...
              {/* Desktop Navigation Links - Hidden on Painting Cleaning, and Packers Movers page */}
              {!isPaintingCleaningPage && !isPackersMoversPage && <nav className="hidden lg:flex items-center space-x-5">
    ...
    {isServicesDropdownOpen && <div className="absolute top-full left-0 w-56 bg-white border border-gray-200 rounded-lg shadow-xl z-[100] mt-2" onMouseEnter={handleServicesHover} onMouseLeave={handleServicesLeave}>
     ...
     Jobs
            ...
            <div className="flex items-center space-x-2 sm:space-x-4">
               {/* Phone Number - Only visible on Painting Cleaning, and Packers Movers page */}
               {(isPaintingCleaningPage || isPackersMoversPage) && <a href="tel:+919036015272" className={`flex items-center px-3 py-2 rounded-lg border transition-all duration-500 ${isScrolled ? 'bg-white text-red-600 border-red-200 hover:bg-red-50' : 'bg-white/10 backdrop-blur-sm text-white border-white/30 hover:bg-white/20'}`}>
                  <svg className="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M2 3a1 1 0 011-1h2.153a1 1 0 01.986.836l.74 4.435a1 1 0 01-.54 1.06l-1.548.773a11.037 11.037 0 006.105 6.105l.774-1.548a1 1 0 011.059-.54l4.435.74a1 1 0 01.836.986V17a1 1 0 01-1 1h-2C7.82 18 2 12.18 2 5V3z" />
                  </svg>
                  <span className="font-medium text-sm">+91 80740 17388</span>
                </a>}
               {/* Post Property Button - Hidden on Painting Cleaning, and Packers Movers page */}
              ...
              {user && <DropdownMenu>
               ...
               {/* Sidebar toggle button - Hidden on Painting Cleaning, and Packers Movers page */}
               {!isPaintingCleaningPage && !isPackersMoversPage && <Button variant="ghost" size="sm" className={`flex items-center space-x-2 p-2 transition-colors duration-500 ${isScrolled ? 'text-gray-800 hover:bg-gray-100' : 'text-white hover:bg-white/10'}`} onClick={() => setIsSidebarOpen(true)}>
                  <Menu size={20} />
        ...
        {/* Sidebar Component - Hidden on Painting Cleaning, and Packers Movers page */}
        {!isPaintingCleaningPage && !isPackersMoversPage && <Sidebar isOpen={isSidebarOpen} onClose={() => setIsSidebarOpen(false)} />}
