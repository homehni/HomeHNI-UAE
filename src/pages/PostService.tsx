...
const PostService = () => {
  ...
  const propertyTypes = ["Apartment/Flat", "Independent House/Villa", "Plot/Land", "Office", "Retail/Shop", "Warehouse", "Industrial", "Co-working", "Others"];
  const serviceOptions = ["Property Management", "Tenant Management", "Rental & Leasing", "Property Valuation", "Legal & Documentation", "Home Interiors", "Home Loan Assistance", "Property Maintenance & Repairs", "NRI Property Assistance", "Sale Advisory", "Buy Advisory", "Due Diligence", "Others"];
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.phone.trim()) newErrors.phone = "Phone number is required";else if (!/^\d{10,15}$/.test(formData.phone.replace(/[^\d]/g, ""))) {
      newErrors.phone = "Phone number must be 10-15 digits";
    }
    if (!formData.email.trim()) newErrors.email = "Email is required";else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email)) {
            ...
            <Card className="p-8">
              <div className="flex flex-col items-center space-y-4">
                ...
                <div className="bg-muted p-4 rounded-lg w-full text-left">
                  ...
                  {formData.budgetRange[0] > 0 && formData.budgetRange[1] > 0 && <p><strong>Budget:</strong> {formatBudgetAmount(formData.budgetRange[0])} - {formatBudgetAmount(formData.budgetRange[1])}</p>}
                  {formData.premiumSelected && <p><strong>Premium Service:</strong> Active</p>}
                </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <Input id="name" value={formData.name} onChange={e => handleInputChange("name", e.target.value)} className="mt-2 h-12" placeholder="Your full name" />
                    {errors.name && <p className="text-sm text-destructive mt-1">{errors.name}</p>}
                  </div>

                  <div>
                    <Label htmlFor="phone" className="text-base font-medium">Phone Number *</Label>
                    <Input id="phone" type="tel" value={formData.phone} onChange={e => handleInputChange("phone", e.target.value)} className="mt-2 h-12" placeholder="Your phone number" />
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <Input id="email" type="email" value={formData.email} onChange={e => handleInputChange("email", e.target.value)} className="mt-2 h-12" placeholder="your.email@example.com" />
                  ...
                  <div>
                    ...
                    <Select value={formData.country} onValueChange={value => handleInputChange("country", value)}>
                      ...
                      <SelectContent>
                        {countryOptions.map(country => <SelectItem key={country.value} value={country.value}>
                            {country.label}
                          </SelectItem>)}
                      </SelectContent>
                    </Select>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    ...
                    handleInputChange("city", ""); // Reset city when state changes
                      ...
                      <SelectContent>
                        {formData.country === "India" && statesData && Object.keys(statesData).map(state => <SelectItem key={state} value={state}>{state}</SelectItem>)}
                  ...
                  <div>
                    ...
                    <Select value={formData.city} onValueChange={value => handleInputChange("city", value)}>
                      ...
                      <SelectContent>
                        ...
                        {cities.length === 0 && formData.state && <SelectItem value="Other">Other (Please specify in notes)</SelectItem>}
                      </SelectContent>
                    </Select>
                ...
                <div>
                  ...
                  handleInputChange("services", []);
                    ...
                    <SelectContent>
                      {intentOptions.map(option => <SelectItem key={option.value} value={option.value}>
                          {option.label}
                        </SelectItem>)}
                    </SelectContent>
                  </Select>
                ...
                {/* Conditional Fields */}
                {["Buy", "Sell", "Lease"].includes(formData.intent) && <div className="transition-all duration-300 ease-in-out">
                ...
                {formData.intent === "Service" && <div className="transition-all duration-300 ease-in-out space-y-4">
                    <div>
                    ...
                    {formData.services.includes("Others") && <div className="transition-all duration-300 ease-in-out">
                        <Label htmlFor="otherService" className="text-base font-medium">Please specify *</Label>
                        <Input id="otherService" value={formData.otherService} onChange={e => handleInputChange("otherService", e.target.value)} className="mt-2 h-12" placeholder="Describe the service you need" />
                ...
                <div>
                  ...
                  <div className="mt-4 space-y-4">
                    <div className="px-4">
                      <Slider value={formData.budgetRange} onValueChange={value => handleInputChange("budgetRange", value)} max={50000000} min={0} step={100000} className="w-full" />
                    </div>
                ...
                <div className="border rounded-lg p-4 bg-muted/30">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <Switch checked={formData.premiumSelected} onCheckedChange={checked => handleInputChange("premiumSelected", checked)} />
                  </div>
                  {formData.premiumSelected && <div className="mt-4 space-y-4 transition-all duration-300 ease-in-out">
                      ...
                      <div>
                        ...
                        <RadioGroup value={formData.paymentMethod} onValueChange={value => handleInputChange("paymentMethod", value)} className="mt-2">
                ...
                <div>
                  ...
                  <Textarea id="notes" value={formData.notes} onChange={e => handleInputChange("notes", e.target.value)} className="mt-2 min-h-[100px]" placeholder="Anything else we should know?" />
                </div>

                {/* Submit Button */}
                <Button type="submit" className="w-full h-12 text-lg" disabled={submissionState.isSubmitting}>
