...
// Update user role
export const updateUserRole = async (userId: string, newRole: 'buyer' | 'seller' | 'admin' | 'user') => {
...
export const hasUserRole = async (role: 'admin' | 'buyer' | 'seller' | 'user'): Promise<boolean> => {
  const { data, error } = await supabase
