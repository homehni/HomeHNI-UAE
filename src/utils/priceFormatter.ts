 ...
 * @param currencySymbol - Currency symbol to use (default: from country config, 'AED' for UAE)
...
export const formatPriceDisplay = (amount: number | string, currencySymbol?: string): string => {
  ...
  const symbol = currencySymbol || (countryConfig.currency === 'AED' ? 'AED' : '₹');
  ...
  if (numAmount >= 10000000) {
    ...
    return `${symbol} ${crores % 1 === 0 ? crores.toFixed(0) : crores.toFixed(1)} Cr`;
  } else if (numAmount >= 100000) {
    // Lacs (1 Lac = 100,000)
    const lacs = numAmount / 100000;
    return `${symbol} ${lacs % 1 === 0 ? lacs.toFixed(0) : lacs.toFixed(1)} Lac`;
