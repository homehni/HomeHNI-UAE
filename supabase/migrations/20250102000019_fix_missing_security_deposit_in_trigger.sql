-- Fix missing security_deposit field in the sync trigger
-- This migration adds the missing security_deposit field to the properties table sync trigger

-- First, drop the existing trigger
DROP TRIGGER IF EXISTS sync_properties_with_submissions ON public.property_submissions;

-- Recreate the trigger function with security_deposit included
...
BEGIN
  ...
  INSERT INTO public.properties AS p (
    ...
    COALESCE((NEW.payload->>'water_storage_facility')::boolean, false),
...
COMMENT ON FUNCTION sync_properties_with_submissions() IS 'Fixed trigger function that now includes security_deposit field mapping from property_submissions to properties table';
