-- Fix auto-approve properties functionality
-- The trigger was hardcoding status = 'pending' instead of using the submission's status
...
BEGIN
  ...
  IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
    ...
    INSERT INTO public.properties AS p (
      ...
      img_array,
      vid_array,
      COALESCE(NEW.status, 'pending'),  -- Use submission's status instead of hardcoded 'pending'
      true,
      true,
    ...
    ON CONFLICT (id) DO UPDATE SET
      ...
      status = COALESCE(EXCLUDED.status, p.status, 'pending'),  -- Respect the new status from submission
      is_visible = true,
      is_featured = true,
      updated_at = now();
...
-- Approve all existing pending properties (one-time cleanup)
