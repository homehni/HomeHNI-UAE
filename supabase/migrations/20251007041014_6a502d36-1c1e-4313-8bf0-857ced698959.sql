-- Update RLS policy to allow public viewing of all visible properties regardless of approval status
-- This allows properties to appear immediately in search results and featured sections
-- Admin can still review, hide, or delete them

DROP POLICY IF EXISTS "Public can view approved and visible properties" ON public.properties;

CREATE POLICY "Public can view all visible properties"
ON public.properties
FOR SELECT
USING (is_visible = true);

-- Update the get_public_properties function to not filter by status
...
RETURNS TABLE(
  ...
  additional_documents jsonb, plot_area_unit text
)
LANGUAGE sql
STABLE
SECURITY DEFINER
...
-- Update get_public_property_by_id to also not filter by status
