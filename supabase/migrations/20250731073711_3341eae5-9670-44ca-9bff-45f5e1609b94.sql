...
CREATE TYPE property_status AS ENUM ('pending', 'active', 'approved', 'rejected', 'deleted');

ALTER TABLE public.properties 
ALTER COLUMN status TYPE property_status USING status::property_status,
ALTER COLUMN status SET DEFAULT 'pending'::property_status;
...
-- Insert admin role for testing (replace with actual admin user ID when available)
