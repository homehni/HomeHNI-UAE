-- Update the property sync trigger to include available_services field
...
DECLARE
  ...
  final_status text;
BEGIN
  -- Extract images and videos arrays
  img_array := ARRAY(
    SELECT jsonb_array_elements_text(NEW.payload->'images')
    WHERE NEW.payload->'images' IS NOT NULL
  );
  
  vid_array := ARRAY(
    SELECT jsonb_array_elements_text(NEW.payload->'videos')
    WHERE NEW.payload->'videos' IS NOT NULL
  );

  -- Determine final status
  CASE 
    WHEN NEW.status = 'draft' THEN 'draft'::text
    WHEN NEW.status = 'pending' THEN 'pending'::text
  ...
  INSERT INTO public.properties AS p (
    ...
    COALESCE(NEW.additional_info->'available_services', '{}'::jsonb)
  )
