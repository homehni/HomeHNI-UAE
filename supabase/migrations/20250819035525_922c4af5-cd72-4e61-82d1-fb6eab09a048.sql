-- Security Fixes - Phase 1: Proper cleanup and data integrity
-- Drop triggers and functions in correct dependency order
...
DROP TRIGGER IF EXISTS audit_property_status_trigger ON public.properties;

-- Now drop functions
DROP FUNCTION IF EXISTS public.prevent_non_admin_status_update() CASCADE;
DROP FUNCTION IF EXISTS public.audit_property_status_change() CASCADE;

-- Update invalid property statuses now that triggers are removed
...
-- Create RLS policies for user role audit log
