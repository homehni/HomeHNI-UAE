-- Seed existing website content into CMS (simplified approach)
...
SELECT * FROM (VALUES
  ('navigation', 'header_nav', 'Main Navigation', '{"logo": "/lovable-uploads/main-logo-final.png", "brand_name": "HomeHNI"}', 'homepage', 'header', 0, true),
  ('footer', 'footer_content', 'Footer Content', '{"company_info": {"name": "HomeHNI", "description": "Your trusted partner in real estate"}}', 'homepage', 'footer', 0, true)
) AS elements(element_type, element_key, title, content, page_location, section_location, sort_order, is_active)
WHERE NOT EXISTS (
  SELECT 1 FROM public.content_elements 
  WHERE element_key = elements.element_key AND page_location = elements.page_location
);