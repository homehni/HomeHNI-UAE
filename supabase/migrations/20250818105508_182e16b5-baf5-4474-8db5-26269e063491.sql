...
-- Add public access to sanitized view
CREATE POLICY "Public can view sanitized properties"
ON public.properties
FOR SELECT
USING (false); -- Block direct access to properties table

-- Phase 2: Add missing database triggers
...
-- Remove existing policies and create more secure ones
...
-- Create secure property-media policies
...
-- Phase 4: Add admin access to leads (if not already exists)
CREATE POLICY IF NOT EXISTS "Admins can view all leads"
ON public.leads
FOR SELECT
USING (public.has_role(auth.uid(), 'admin'));

CREATE POLICY IF NOT EXISTS "Admins can update all leads"
ON public.leads
FOR UPDATE
USING (public.has_role(auth.uid(), 'admin'));