-- Add missing preferred_tenant field to the sync trigger
-- This migration adds the preferred_tenant field to the properties table sync trigger

-- First, drop the existing trigger
DROP TRIGGER IF EXISTS sync_properties_with_submissions ON public.property_submissions;

-- Recreate the trigger function with preferred_tenant included
...
BEGIN
  ...
  INSERT INTO public.properties AS p (
    ...
    additional_info,
    -- Security deposit and preferred tenant fields
    security_deposit, preferred_tenant
  ) VALUES (
    NEW.id,
    ...
    COALESCE((NEW.payload->>'gated_project')::boolean, false),
    NEW.payload->>'floors_allowed',
