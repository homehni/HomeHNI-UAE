-- Update the sync_properties_with_submissions trigger to include additional_info field
...
DECLARE
  ...
  final_status TEXT;
BEGIN
  -- Handle images array from payload
  IF NEW.payload->'images' IS NOT NULL AND jsonb_typeof(NEW.payload->'images') = 'array' THEN
    SELECT ARRAY(
  ...
  -- Handle videos array from payload
  IF NEW.payload->'videos' IS NOT NULL AND jsonb_typeof(NEW.payload->'videos') = 'array' THEN
    SELECT ARRAY(
  ...
  final_status := CASE 
    ...
    WHEN NEW.status = 'rejected' THEN 'inactive'::text
    WHEN NEW.status = 'pending' AND COALESCE(NEW.payload->>'auto_approve', 'false') = 'true' THEN 'active'::text
    WHEN NEW.status = 'pending' THEN 'inactive'::text
    WHEN NEW.status = 'sold' THEN 'sold'::text
    WHEN NEW.status = 'rented' THEN 'rented'::text
    ELSE 'active'::text
  END;
  ...
  INSERT INTO public.properties AS p (
    ...
    -- PG/Hostel services from additional_info
    COALESCE(NEW.payload->'additional_info'->'available_services', '{}'::jsonb),
    -- Additional info for flexible data storage
    COALESCE(NEW.payload->'additional_info', '{}'::jsonb)
  )
